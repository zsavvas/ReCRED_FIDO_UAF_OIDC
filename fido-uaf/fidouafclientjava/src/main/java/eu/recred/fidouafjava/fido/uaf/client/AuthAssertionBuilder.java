...
import eu.recred.fidouafjava.fido.uaf.crypto.Asn1;
import eu.recred.fidouafjava.fido.uaf.crypto.BCrypt;
import eu.recred.fidouafjava.fido.uaf.crypto.KeyCodec;
import eu.recred.fidouafjava.fido.uaf.crypto.NamedCurve;
import eu.recred.fidouafjava.fido.uaf.crypto.SHA;
import eu.recred.fidouafjava.fido.uaf.msg.AuthenticationResponse;
import eu.recred.fidouafjava.fido.uaf.tlv.TagsEnum;
...
//				.decodeBase64(TestData.TEST_PUB_KEY));
		...
		PublicKey pub =
				KeyCodec.getPubKey(Base64.decode(prefs.getSettingsParam("pub"), Base64.URL_SAFE));
		PrivateKey priv =
				KeyCodec.getPrivKey(Base64.decode(prefs.getSettingsParam("priv"), Base64.URL_SAFE));
...
//				.decodeBase64(TestData.TEST_PRIV_KEY));
	...
	private byte[] getKeyId() throws IOException {
		ByteArrayOutputStream byteout = new ByteArrayOutputStream();
		String keyId = prefs.getSettingsParam("keyId");
