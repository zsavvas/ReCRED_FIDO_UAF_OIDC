...
import eu.recred.fidouafjava.fido.uaf.msg.asm.ASMRequest;
import eu.recred.fidouafjava.fido.uaf.msg.asm.Request;
import eu.recred.fidouafjava.fido.uaf.msg.asm.obj.AuthenticateIn;
...
public class Auth {
	...
	public String getUafMsgRequest (String facetId, boolean isTrx){
		String serverResponse = getAuthRequest();
		return OpUtils.getUafRequest(serverResponse,facetId,isTrx);
	}
	
	public String clientSendResponse (String uafMessage){
		return  OpUtils.clientSendRegResponse(uafMessage,Preferences.getInstance().getServer() + Preferences.getInstance().getAuthResponseEndpoint());
	}

	private String getAuthRequest() {
		String url = Preferences.getInstance().getServer() + Preferences.getInstance().getAuthRequestEndpoint();
	...
	private AuthenticateIn getAuthenticateIn() {
		...
		try{
			...
			ret.keyIDs[0] = Preferences.getInstance().getSettingsParam("keyID");
	...
	public void freezeAuthResponse (AuthenticationRequest authRequest){
		...
		Preferences.getInstance().setSettingsParam("authResponse", json);
	}
	...
	public String sendAuthResponse (String authOut){
		...
		String res = Curl.postInSeparateThread(Preferences.getInstance().getServer() + Preferences.getInstance().getAuthResponseEndpoint(), headerStr , json);
		return res;
	}
	...
	private String getAuthResponseForSending(String authOut) {
		String ret = null;
		try{
		AuthenticationResponse authResponse = gson.fromJson(Preferences.getInstance().getServer() + Preferences.getInstance().getSettingsParam("authResponse"), AuthenticationResponse.class);
