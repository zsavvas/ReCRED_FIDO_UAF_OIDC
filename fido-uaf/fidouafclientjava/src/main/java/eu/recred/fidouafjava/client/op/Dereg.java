...
import eu.recred.fidouafjava.fido.uaf.msg.asm.obj.DeregisterIn;
...
public class Dereg {
	...
	public String getUafMsgRequest (){
		...
		try {
			...
			Preferences.getInstance().setSettingsParam("deregMsg", forSending);
//			post(forSending);
	...
	public ASMRequest<DeregisterIn> getAsmRequest(int authenticatorIndex) {
		...
		arg.appID = Preferences.getInstance().getSettingsParam("appID");
		arg.keyID = Preferences.getInstance().getSettingsParam("keyID");
		ret.args = arg;
	...
	public void recordKeyId(String registrationsOut) {
		...
		try {
			...
			Preferences.getInstance().setSettingsParam("keyID", keyId);
		} catch (JSONException e) {
	...
	public DeregistrationRequest getDereg() {
		try {
			...
			dereg.header.appID = Preferences.getInstance().getSettingsParam("appID");
			dereg.authenticators = new DeregisterAuthenticator[1];
			DeregisterAuthenticator deregAuth = new DeregisterAuthenticator();
			deregAuth.aaid = Preferences.getInstance().getSettingsParam("AAID");
			String tmp = Preferences.getInstance().getSettingsParam("keyID");
	...
	private String post(DeregistrationRequest regResponse) {
		...
		return Curl.postInSeparateThread(Preferences.getInstance().getServer() + Preferences.getInstance().getDeregEndpoint(), header , json);
	}
